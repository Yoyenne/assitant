	...
	<div class="search-container">
		...
		<div class="carousel-wrapper">
			...
			<div class="carousel-indicators">
				<span v-for="(_, index) in carouselImages" :key="index" :class="{ active: index === currentIndex }"
		...
		<div class="upload-section" id="image-upload-section">
			...
			<input id="image-upload" type="file" accept="image/*" class="upload-input" @change="onImageUpload" />
		...
		<div class="upload-section" id="text-upload-section">
			...
			<textarea id="text-upload" class="text-input" placeholder="请输入文字内容，描述您当前的心情..."
...
export default defineComponent({
	...
	setup(_, { emit }) {
		...
		const onSubmitImage = async () => {
			...
			try {
				const res = await musicApi.recommendByImage(imageFile.value);

				const musicStore = useMusicStore();
				musicStore.setMusicInfo('image', imageFile.value);
				musicStore.musicList = res.recommended_songs; 
